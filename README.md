说明：
本插件是一个简单的分页插件(基于bootstrap3)
使用方法见 page-test.html文件

注意：
    1、本插件还有存在的bug，当pageNumSize参数为偶数的时候会有问题，这个问题没有作处理，以后更正。
    ...
    2、关于和实际应用集成：
       ...
       Article.page() 是页面首次加载的时候初始化分页用的，Article.list()是点击分页上标签查询用的，只有查询条件，可以直接
       从页面input的输入框取，也可以像传递(参数：conditions)
